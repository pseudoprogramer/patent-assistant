...
def build_prompt(question: str, cards: List[Dict[str, Any]]) -> str:
    evidence = json.dumps([compact_card(card) for card in cards], ensure_ascii=False, indent=2)
...
Use only the provided patent cards. Do not invent patents, claims, or evidence.
If the evidence is weak or the top results look noisy, say so clearly.

User question:
{question}

Retrieved patent cards:
{evidence}
